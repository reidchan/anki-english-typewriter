...
// 延迟获取，避免 SSR hydration 错误
let cachedIsMacOS: boolean | null = null;

export const getIsMacOS = (): boolean => {
  if (typeof window === "undefined") return false;
  if (cachedIsMacOS === null) {
    cachedIsMacOS = navigator.userAgent.indexOf("Macintosh") !== -1;
  }
  return cachedIsMacOS;
};

// 为了向后兼容，保留这个导出（但服务端始终返回 false）
export const IS_MAC_OS =
  typeof window !== "undefined" &&
  navigator.userAgent.indexOf("Macintosh") !== -1;

// 服务端和客户端初始渲染都使用 "Ctrl"，避免 hydration 错误
// 组件可以在 useEffect 中调用 getIsMacOS() 来获取真实值并更新
